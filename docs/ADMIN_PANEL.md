...
### Problem
The `super-admin` role needs to be securely assigned to the first administrative user without requiring manual database manipulation.

### Solution
A one-time bootstrap process securely creates the Cognito identity and links it to the database with super-admin privileges:
...
4. Calls `POST /auth/api/v1/admin/bootstrap/new-super-admin` with `{ cognitoSub, email }`.
5. The backend safely creates the user record and directly attaches the `super-admin` role.

**Endpoint protection:** The bootstrap endpoint is protected by an internal API key (`X-Internal-Api-Key` header) securely generated by Terraform and stored in AWS SSM. It is not protected by JWT because the admin does not have a valid session yet at bootstrap time.
...
sequenceDiagram
    ...
    AuthService->>DB: Assign super-admin role
    AuthService-->>Script: 200 OK (admin detail)
```

### Execution Environments

The script automatically fetches the correct configuration URLs and keys depending on the target environment. You can control this by setting the `ENVIRONMENT` variable before executing the script.

**For Budget Deployments (Default):**
```bash
ENVIRONMENT=budget ./scripts/bootstrap-system-admin.sh
```
*(Fetches secure keys and URLs directly from AWS SSM Parameter Store)*

**For Local Development:**
```bash
ENVIRONMENT=local ./scripts/bootstrap-system-admin.sh
```
*(Uses local Docker network settings and `.env` variables)*

**For Production Deployments:**
```bash
ENVIRONMENT=prod ./scripts/bootstrap-system-admin.sh
```
