...
```bash
# Use AWS CLI
aws logs tail /ecs/GSTbuddies-prod/gateway-service --follow

# Or CloudWatch Console
# Navigate to: CloudWatch → Log Groups → /ecs/GSTbuddies-prod/*
```
...
docker exec -it GSTbuddies-postgres-dev psql -U postgres -d gst_buddy
...
docker exec -it GSTbuddies-postgres-dev psql -U postgres -d gst_buddy -c \
