...
|---------|----------|-------------|
| **Auth Service** | `GSTbuddies` (schema: `public`) | User identities, roles, and permissions (if persisting locally) |
| **Backend Service** | `backendgst_buddy` (or shared `GSTbuddies`) | Business data (Rule 37 calculation runs) |

> **Note:** For the "Lite" template, both services typically share the same PostgreSQL instance and can share the same database (`GSTbuddies`) for simplicity, but strictly separate tables.
