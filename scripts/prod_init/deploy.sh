#!/bin/bash
# ============================================================================
# Production Deployment (prod_init) - One Shot (JAR Rsync Mode)
# ============================================================================
...
# ============================================================================
# Branch Guard: Ensure we are on the prod branch
# ============================================================================
CURRENT_BRANCH=$(git branch --show-current)
if [ "$CURRENT_BRANCH" != "prod" ]; then
    echo -e "\033[0;31m[ERROR]\033[0m This script MUST be run from the 'prod' branch."
    echo -e "Current branch is: \033[1;33m$CURRENT_BRANCH\033[0m"
    exit 1
fi
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🚀 Production Deployment (prod_init)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
FRONTEND_URL=$(terraform output -raw frontend_url 2>/dev/null || echo "https://gstbuddies.com")
API_GATEWAY_URL=$(terraform output -raw api_url 2>/dev/null || echo "https://api.gstbuddies.com")
...
if [ -n "$AMPLIFY_APP_ID" ] && [ "$AMPLIFY_APP_ID" != "" ]; then
    log_info "Triggering Amplify build for app: $AMPLIFY_APP_ID (branch: prod)"
    
    JOB_OUTPUT=$(aws amplify start-job --app-id "$AMPLIFY_APP_ID" --branch-name prod --job-type RELEASE --region "$AWS_REGION" 2>/dev/null || echo "")
    if [ -n "$JOB_OUTPUT" ]; then
        ...
        while true; do
            JOB_STATUS=$(aws amplify get-job --app-id "$AMPLIFY_APP_ID" --branch-name prod --job-id "$JOB_ID" --region "$AWS_REGION" 2>/dev/null | jq -r '.job.summary.status' || echo "UNKNOWN")
        ...
        echo "  aws amplify start-job --app-id $AMPLIFY_APP_ID --branch-name prod --job-type RELEASE"
    fi
else
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log_success "Production Deployment (prod_init) Complete!"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "🌐 Access:"
echo "  Frontend: $FRONTEND_URL (branch: prod)"
