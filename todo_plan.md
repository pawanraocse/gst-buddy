...
- [x] **C1** — Parser `gstr3b_pdf.py`: implement `_parse_table_3_1()` + `_parse_table_6_1()`; extend `_parse_text()` return shape with `table_3_1`, `table_6_1` [MODIFY]
- [x] **C2** — Parser `gstr1_pdf.py`: implement `_parse_liability_summary()` + `_parse_invoices()`; extend return shape with `liability_summary`, `invoices[]` [MODIFY]
- [x] **C3** — Parser tests: add fixtures under `gst-parser/tests/fixtures/` + unit tests for both extractors [NEW]
- [x] **C4** — Domain: `domain/recon/Gstr1Vs3bInput.java` + `Gstr1Vs3bResult.java` + `ReconDelta.java` records [NEW]
- [x] **C5** — Service: `domain/recon/Gstr1Vs3bReconciliationService.java` — pure calculator, delta + severity classification [NEW]
- [x] **C6** — Rule: `engine/rules/Gstr1Vs3bReconciliationRule.java` (`RECON_1_VS_3B`, order=50, requires `{GSTR_1, GSTR_3B}`) [NEW]
- [x] **C7** — Resolver: `engine/resolvers/Gstr1Vs3bReconciliationInputResolver.java` — matches docs by `taxPeriod`, extracts liability totals [NEW]
- [x] **C8** — Domain: `domain/gstr1/LateReportingGstr1Input.java` + `LateReportingGstr1Result.java` + `BelatedInvoice.java` records [NEW]
- [x] **C9** — Service: `domain/gstr1/LateReportingGstr1CalculatorService.java` — Section 50(1) interest at 18% p.a. per invoice, day-count from original due date → declared date [NEW]
- [x] **C10** — Rule: `engine/rules/LateReportingGstr1Rule.java` (`LATE_REPORTING_GSTR1`, order=20, requires `{GSTR_1}`) [NEW]
- [x] **C11** — Resolver: `engine/resolvers/LateReportingGstr1InputResolver.java` — extracts `invoices[]` from document, derives expected period from `invoice_date` [NEW]
- [x] **C12** — Flyway `V6__recon_config.sql`: `recon_tolerance_config` table (per-tenant override of match tolerance in ₹ + %) — seeded with default (±1 ₹ / ±0.01%) [NEW]
- [x] **C13** — Entity: `entity/ReconToleranceConfig.java` + repository; wire into `ContextEnricher` → `SharedResources.reconTolerance` [NEW]
- [x] **C14** — Extend `SharedResources` record with `reconTolerance` field (backward-compat default) [MODIFY]
- [x] **C15** — Unit tests: `Gstr1Vs3bReconciliationServiceTest` (match / under-report / over-report / multi-period / tolerance) — ≥ 6 tests [NEW]
- [x] **C16** — Unit tests: `LateReportingGstr1CalculatorServiceTest` (on-time / 1-day belated / multi-month belated / nil invoices / mixed) — ≥ 5 tests [NEW]
- [x] **C17** — Unit tests: both resolvers (`Gstr1Vs3bReconciliationInputResolverTest`, `LateReportingGstr1InputResolverTest`) — missing-doc, period-mismatch, happy-path [NEW]
- [x] **C18** — Frontend: `features/audit/components/unlockable-rules-panel/` (PrimeNG `Card` + `Chip`) — shows "Upload GSTR-3B to unlock RECON_1_VS_3B" [NEW]
- [x] **C19** — Frontend: extend `comprehensive-report` view with a **Reconciliation** section rendering `ReconDelta` rows in PrimeNG `Table` with severity badges [MODIFY]
- [x] **C20** — Frontend: audit service DTO types — add `reconFindings`, `belatedInvoiceFindings` to `UploadResult` type [MODIFY]
- [x] **C21** — E2E smoke: upload matched GSTR-1 + GSTR-3B pair → assert one `RECON_1_VS_3B` finding; upload GSTR-1 with belated invoice → assert `LATE_REPORTING_GSTR1` finding with non-zero interest [NEW]
...
- [x] **D1** — Domain: `domain/pos/PosValidationInput.java` + `PosValidationResult.java` + `PosMismatch.java` records [NEW]
- [x] **D2** — Service: `domain/pos/PosValidationService.java` — for each invoice, compare `place_of_supply` state vs supplier state vs tax split (IGST if inter-state, CGST+SGST if intra-state) [NEW]
- [x] **D3** — Rule + Resolver: `engine/rules/PosValidationGstr1Rule.java` (order=50) + `engine/resolvers/PosValidationGstr1InputResolver.java` [NEW]
- [x] **D4** — Unit tests: service (intra-state correct / intra-state wrong split / inter-state correct / inter-state wrong / SEZ / export) — ≥ 6 tests [NEW]

**D-B · RULE_86B_RESTRICTION**
- [x] **D5** — Flyway `V7__rule86b_config.sql`: `rule_86b_threshold_config` reference table (turnover threshold ₹, cash-ledger-% floor, effective-from date) — seeded with Notification 94/2020-CT defaults [NEW]
- [x] **D6** — Domain: `domain/rule86b/Rule86bInput.java` + `Rule86bResult.java` records [NEW]
- [x] **D7** — Service: `domain/rule86b/Rule86bCalculationService.java` — applies exemptions (govt entities, ≥ ₹1 lakh IT refund, export with refund claim) before flagging [NEW]
- [x] **D8** — Rule + Resolver: `engine/rules/Rule86bAuditRule.java` (order=40) + `engine/resolvers/Rule86bInputResolver.java` [NEW]
- [x] **D9** — Extend `SharedResources` with `rule86bConfig`; load in `ContextEnricher` [MODIFY]
- [x] **D10** — Unit tests: evaluator (below threshold / above threshold compliant / above threshold breach / each exemption path) — ≥ 6 tests [NEW]

**D-C · SUPPLIER_RISK_2A**
- [x] **D11** — Parser: `gst-parser/app/engines/gstr2a_pdf.py` (+ `json_engine.py` route) — extract supplier GSTINs + invoice totals [NEW]
- [x] **D12** — Flyway `V8__gstin_status_cache.sql`: `gstin_status_cache` (gstin PK, status, last_filing_period, fetched_at, ttl_expires_at) [NEW]
- [x] **D13** — Entity + repo: `entity/GstinStatusCache.java` + `GstinStatusCacheRepository` [NEW]
- [x] **D14** — Service: `service/GstinStatusLookupService.java` — TTL-aware read-through cache; portal client stubbed behind an interface for this phase [NEW]
- [x] **D15** — Domain: `domain/risk/SupplierRiskInput.java` + `SupplierRiskResult.java` records [NEW]
- [x] **D16** — Service: `domain/risk/SupplierRiskScoringService.java` — scores each supplier (non-filer / cancelled / composition mismatch / ITC at risk) [NEW]
- [x] **D17** — Rule + Resolver: `engine/rules/SupplierRiskAuditRule.java` (order=60) + `engine/resolvers/SupplierRiskInputResolver.java` [NEW]
- [x] **D18** — Extend `ContextEnricher` to batch-load cached GSTIN statuses into `SharedResources` [MODIFY]
- [x] **D19** — Unit tests: scorer + cache service (hit / miss / expired / portal failure fallback) — ≥ 8 tests [NEW]

- [x] **D20** — Parser: `gst-parser/app/engines/gstr2b_pdf.py` — extract ITC rows with invoice_date + tax values [NEW]
- [x] **D21** — Domain: `domain/itc/Section16_4Input.java` + `Section16_4Result.java` + `ExpiredItc.java` records [NEW]
- [x] **D22** — Service: `domain/itc/Section16_4CalculationService.java` — deadline = `30-Nov of FY following invoice_date.fy` (or annual return filing date, whichever earlier per Section 16(4)) [NEW]
- [x] **D23** — Rule + Resolver: `engine/rules/Section16_4AuditRule.java` (order=30) + `engine/resolvers/Section16_4InputResolver.java` [NEW]
- [x] **D24** — Unit tests: service (within window / exactly on deadline / past deadline / FY boundary / annual-return-filed-earlier edge) — ≥ 6 tests [NEW]

**Shared D-phase tasks**
- [x] **D25** — Frontend: per-rule report cards for the 4 new rules in `comprehensive-report` view — one PrimeNG `Panel` per rule with severity-coloured chips [MODIFY]
- [x] **D26** — Update `DocumentType` enum usage in `AuditAnalyzeController` preview endpoint to include `GSTR_2A`, `GSTR_2B`, `PURCHASE_REGISTER` [MODIFY]
- [x] **D27** — Cross-check each rule's LLD against `resources/RULE ENGINE/*/PRD to SD.docx` before PR merge [VERIFY]
- [x] **D28** — E2E smoke: upload GSTR-3B with 0% cash → expect `RULE_86B_RESTRICTION` BREACHED; upload GSTR-2A with cancelled GSTIN → expect `SUPPLIER_RISK_2A` HIGH finding [NEW]
...
- [x] **E1** — Parser: extend `gst-parser/app/engines/` — `gstr9_pdf.py` engine to extract Table 4 (outward supplies), Table 5 (exempted/nil), Table 9 (tax payable/paid) as structured JSON [NEW]
- [x] **E2** — Parser tests: fixtures + unit tests for GSTR-9 table extraction [NEW]
- [x] **E3** — Domain: `domain/recon/Gstr1Vs3bVs9Input.java` + `Gstr1Vs3bVs9Result.java` + `ThreeWayReconDelta.java` records [NEW]
- [x] **E4** — Service: `domain/recon/Gstr1Vs3bVs9ReconciliationService.java` — 3-way delta computation: GSTR-1 (monthly aggregate) vs GSTR-3B (monthly aggregate) vs GSTR-9 (annual); delta per tax head (CGST/SGST/IGST/CESS); severity by delta % using tolerance from `SharedResources` [NEW]
- [x] **E5** — Rule + Resolver: `engine/rules/Gstr1Vs3bVs9ReconciliationRule.java` (`RECON_1_VS_3B_VS_9`, order=50, requires `{GSTR_1, GSTR_3B, GSTR_9}`) + `engine/resolvers/Gstr1Vs3bVs9InputResolver.java` — aggregates all GSTR-1 + GSTR-3B docs by `taxPeriod` for the FY, pairs with GSTR-9 annual totals [NEW]
- [x] **E6** — Unit tests: reconciliation service (all match / monthly under-report compensated in annual / systematic under-report / over-report / tolerance boundary / multi-rate breakdown) — ≥ 6 tests [NEW]

**E-B · ITC_RECO_2B**
- [x] **E7** — Parser: extend `gst-parser/app/engines/gstr2b_pdf.py` (from Phase D-D) — additionally extract supplier-wise ITC rows: `supplier_gstin`, `invoice_no`, `invoice_date`, `taxable_value`, `cgst`, `sgst`, `igst`, `cess`, `itc_available` flag [MODIFY]
- [x] **E8** — Parser: `gst-parser/app/engines/purchase_register_excel.py` — new Excel engine for purchase register: column alias mapping (Tally/Busy/portal header variations), extract `supplier_gstin`, `invoice_no`, `invoice_date`, `taxable_value`, `cgst`, `sgst`, `igst`, `cess`, `rcm_flag` [NEW]
- [x] **E9** — Parser tests: fixtures + unit tests for purchase register Excel parsing (Tally format, Busy format, portal format) [NEW]
- [x] **E10** — Domain: `domain/itc/ItcRecoInput.java` + `ItcRecoResult.java` + `ItcMismatch.java` records (mismatch types: MISSING_IN_2B, MISSING_IN_BOOKS, AMOUNT_MISMATCH, GSTIN_MISMATCH) [NEW]
- [x] **E11** — Service: `domain/itc/ItcReconciliationService.java` — fuzzy match on (`supplier_gstin` exact + `invoice_no` normalised + `taxable_value` within tolerance ₹); classify mismatches; compute ITC at risk per Section 16(2)(aa) [NEW]
- [x] **E12** — Rule + Resolver: `engine/rules/ItcReco2bRule.java` (`ITC_RECO_2B`, order=50, requires `{GSTR_2B, PURCHASE_REGISTER}`) + `engine/resolvers/ItcReco2bInputResolver.java` [NEW]
- [x] **E13** — Unit tests: reconciliation service (full match / missing in 2B / missing in books / amount within tolerance / amount outside tolerance / duplicate invoice / GSTIN typo) — ≥ 7 tests [NEW]

**E-C · RCM_RECO_3B**
- [x] **E14** — Parser: extend `gstr3b_pdf.py` — ensure Table 3.1(d) (inward supplies attracting RCM) is emitted as `table_3_1_d` with `taxable_value`, `cgst`, `sgst`, `igst`, `cess` [MODIFY]
- [x] **E15** — Domain: `domain/rcm/RcmRecoInput.java` + `RcmRecoResult.java` + `RcmMismatch.java` records (mismatch types: UNDECLARED_RCM, OVER_DECLARED_RCM, AMOUNT_MISMATCH) [NEW]
- [x] **E16** — Service: `domain/rcm/RcmReconciliationService.java` — compare GSTR-3B Table 3.1(d) aggregate vs purchase register RCM-flagged invoices aggregate; per-supplier drill-down; ITC-at-risk = undeclared RCM amount (eligible ITC missed + potential demand under Section 9(3)/9(4)) [NEW]
- [x] **E17** — Rule + Resolver: `engine/rules/RcmReco3bRule.java` (`RCM_RECO_3B`, order=50, requires `{GSTR_3B, PURCHASE_REGISTER}`) + `engine/resolvers/RcmReco3bInputResolver.java` [NEW]
- [x] **E18** — Unit tests: reconciliation service (exact match / under-declared RCM / over-declared / no RCM invoices in register / mixed RCM + forward charge / tolerance edge) — ≥ 6 tests [NEW]

**Shared E-phase tasks**
- [DONE] **E19** — Frontend: reconciliation comparison views in `comprehensive-report` — side-by-side delta tables for 3-way recon, ITC mismatch list with severity badges, RCM gap summary [MODIFY]
- [DONE] **E20** — Frontend: DTO types — add `threeWayReconFindings`, `itcMismatches`, `rcmMismatches` to `UploadResult` type [MODIFY]
- [DONE] **E21** — Cross-check each rule's LLD against PRDs: `resources/RULE ENGINE/1 vs 3B vs 9/PRD for SD.docx`, `resources/RULE ENGINE/ITC RECO/`, `resources/RULE ENGINE/RCM reco in 3B/` before PR merge [VERIFY]
- [DONE] **E22** — E2E smoke: upload GSTR-1 + GSTR-3B + GSTR-9 → assert `RECON_1_VS_3B_VS_9` finding with delta breakdown; upload GSTR-2B + Purchase Register → assert `ITC_RECO_2B` mismatches; upload GSTR-3B + Purchase Register (with RCM invoices) → assert `RCM_RECO_3B` gap finding [NEW]
