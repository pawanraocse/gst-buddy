...
class Gstr2aPdfExtractor(BaseExtractor):
    """
    Extracts metadata and Supplier rows from Government GSTR-2A PDFs.
    """
    def extract(self, content: bytes, json_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not result.get("gstin"):
            logger.warning("pdfplumber returned empty result for GSTR-2A; attempting PyMuPDF fallback.")
    ...
    def _extract_with_pdfplumber(self, content: bytes) -> Dict[str, Any]:
        ...
        try:
            with pdfplumber.open(io.BytesIO(content)) as pdf:
                if not pdf.pages:
                    self.add_warning("GSTR-2A PDF has no pages.")
                    return result
                
                text = pdf.pages[0].extract_text() or ""
                result = self._parse_text(text)
                
                result["suppliers"] = self._parse_tables(pdf.pages)
        except Exception as e:
            self.add_warning(f"pdfplumber error for GSTR-2A: {str(e)}")
        return result
    ...
    def _extract_with_pymupdf(self, content: bytes) -> Optional[Dict[str, Any]]:
        try:
            ...
            self.add_warning(f"PyMuPDF fallback failed for GSTR-2A: {str(e)}")
            return None
            ...
            for table in tables:
                for row in table:
                    ...
                    if re.match(r"^[0-9A-Z]{15}$", row_clean[0]):
                        ...
                        if supplier_gstin not in suppliers:
                            suppliers[supplier_gstin] = {
                        ...
                        try:
                            # We assume the second column might be reverse charge flag in some tables, 
                            # but let's safely map to invoice
                            suppliers[supplier_gstin]["invoices"].append({
                                ...
                                "reverse_charge": "N" # Simplified stub
                            })
                        except Exception:
                            pass
