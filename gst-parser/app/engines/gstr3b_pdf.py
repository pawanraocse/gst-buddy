...
class Gstr3bPdfExtractor(BaseExtractor):
    """
    Extracts metadata from Government GSTR-3B PDFs.
    ...
    Extracted fields (all from first page):
      ...
      - arn_date     : Filing date (ISO 8601, CRITICAL for Section 47(2) late fee
                       and Section 50(1) interest computation)
      - tax_period   : e.g. "March" / "April"
      - financial_year: e.g. "2024-25"

    Future (Phase B / table extraction):
      - Table 3.1    : Outward supplies taxable summary (for 1-vs-3B reconciliation)
      - Table 4      : ITC details (available, reversed, net)
    """
    ...
    def _extract_with_pdfplumber(self, content: bytes) -> Dict[str, Any]:
        ...
        try:
            with pdfplumber.open(io.BytesIO(content)) as pdf:
                ...
                # Phase B: extract Table 3.1 (outward supply summary)
                # tables = pdf.pages[0].extract_tables()
                # result["table_3_1"] = self._parse_table_3_1(tables)
